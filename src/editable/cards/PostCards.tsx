  ...
  return (
    <Link href={href} className={`group block min-w-0 overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className="grid lg:grid-cols-[1.08fr_0.92fr]">
        <div className="aspect-[16/11] overflow-hidden bg-[var(--slot4-media-bg)] lg:aspect-auto">
        ...
        <div className="flex flex-col justify-center p-8 lg:p-12">
          ...
          <h3 className="mt-4 max-w-3xl text-[32px] font-bold leading-[1.15] text-[var(--slot4-page-text)] sm:text-[38px]">{post.title}</h3>
          <p className="mt-4 max-w-2xl text-base leading-[1.75] text-[var(--slot4-muted-text)]">{getEditableExcerpt(post, 190)}</p>
          <span className={`${dc.button.primary} mt-7 w-fit`}>
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]" />
        <span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--slot4-page-text)]">No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
        <p className={`${dc.type.eyebrow} ${pal.accentText}`}>{getEditableCategory(post)}</p>
        <h3 className={`mt-3 line-clamp-3 text-xl font-bold leading-tight ${pal.panelText}`}>{post.title}</h3>
    ...
    <Link href={href} className={`group block min-w-0 ${dc.surface.soft} p-5 ${dc.motion.lift}`}>
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          <h3 className={`mt-2 line-clamp-2 text-xl font-bold leading-tight ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-4 ${dc.motion.lift} sm:grid-cols-[220px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        ...
        <h2 className={`mt-3 line-clamp-3 text-2xl font-bold leading-tight ${pal.panelText} sm:text-3xl`}>{post.title}</h2>
        <p className={`mt-4 line-clamp-3 text-sm leading-7 ${pal.softMutedText}`}>{getEditableExcerpt(post, 180)}</p>
        <span className={`mt-5 inline-flex items-center gap-2 text-sm font-bold ${pal.panelText}`}>Open entry <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
